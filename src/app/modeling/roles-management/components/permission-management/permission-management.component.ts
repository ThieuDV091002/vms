...
import { PermissionsService as AppPermissionsService } from '@proxy/services';
...
export class PermissionManagementComponent
  ...
  protected readonly appPermissionsService = inject(AppPermissionsService);
  @Input()
  ...
  isRoleManagementChecked(): boolean {
    return this.selectedGroupPermissions.some(permission => permission.name.startsWith('AbpIdentity.Roles') && permission.isGranted);
  }

  isUserManagementChecked(): boolean {
    return this.selectedGroupPermissions.some(permission => permission.name.startsWith('AbpIdentity.Users') && permission.isGranted);
  }

  onRoleManagementChange(event: Event) {
    const checked = (event.target as HTMLInputElement).checked;
    this.selectedGroupPermissions.forEach(permission => {
      if (permission.name.startsWith('AbpIdentity.Roles')) {
  ...
  onUserManagementChange(event: Event) {
    const checked = (event.target as HTMLInputElement).checked;
    this.selectedGroupPermissions.forEach(permission => {
      if (permission.name.startsWith('AbpIdentity.Users')) {
        permission.isGranted = checked;
        this.updatePermission(permission);
      }
    });
  ...
  updatePermission(updatedPermission: PermissionGrantInfoDto) {
    const index = this.permissions.findIndex(permission => permission.name === updatedPermission.name);
    if (index !== -1) {
      this.permissions[index].isGranted = updatedPermission.isGranted;
    }
  }
  ...
  submit() {
    ...
    this.appPermissionsService
      ...
      .subscribe(() => {
        this.toasterService.success('::LABEL_PermissionUpdated', '', {
          messageLocalizationParams: [this.providerKey],
        });
        if (this.shouldFetchAppConfig()) {
  ...
  openModal() {
    ...
    return this.appPermissionsService.get(this.providerName, this.providerKey).pipe(
      tap((permissionRes: GetPermissionListResultDto) => {
        permissionRes.groups = permissionRes.groups.sort((a, b) => a.name.localeCompare(b.name));;
