...
import { AttractionDto } from '@apis/vms/dtos/attraction';
import { AttractionService } from '@apis/vms/services';
...
@Component({
  ...
  providers: [
      ListService,
      {
        ...
        useValue: 'AttractionComponent',
      },
    ],
})
export class AttractionComponent implements OnInit {
  selected: AttractionDto;
  isModalVisible: boolean;
  data: PagedResultDto<AttractionDto> = { items: [], totalCount: 0 };
  form: FormGroup;
  columns = [
  ...
  ngOnInit(): void {
    this.hookToQuery();
    this.localizationService.get('vms::LABEL_Attraction').subscribe(data => {
      this.info = data
    });
  }
  ...
  private hookToQuery() {
    ...
    add() {
      this.selected = {} as AttractionDto;
      this.buildForm();
