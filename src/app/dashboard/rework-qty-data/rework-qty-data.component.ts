...
import { ProductService, ReworkDataService, WorkOrderDataService } from '@apis/general/production-review';
...
@Component({
  selector: 'app-rework-qty-data',
  templateUrl: './rework-qty-data.component.html',
  styleUrl: './rework-qty-data.component.scss'
})
export class ReworkQtyDataComponent implements OnInit, OnChanges {
  @Input() openAddReworkQtyData: false;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (changes.openAddReworkQtyData && changes.openAddReworkQtyData.currentValue) {
      this.open();
    }
  ...
  buildForm() {
    this.form = this.fb.group({
      ...
      reworkQty: [0, [Validators.required, this.qtyGreaterThanZeroValidator()]],
      operation: [undefined]
    });
  ...
  save() {
    ...
    const requestBody = {
      ...
      recordTimestamp: new Date(this.roundDownToNearestFiveMinutes(this.form.controls['recordTimestamp'].value)).toISOString(),
      workOrder: this.form.controls['workOrder'].value,
      productId: this.form.controls['product'].value,
      reworkQty: this.form.controls['reworkQty'].value,
      refSourceId: '',
    ...
    this.modalBusy = true;
    this.reworkDataService.create(requestBody).pipe(finalize(() => { this.modalBusy = false; })).subscribe(() => {
