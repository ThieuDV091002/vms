...
if "%skipProxy%"=="false" (
  rd /s /q src\apis\corporate
  abp generate-proxy -t ng --api-name corporate --entry-point src/apis/corporate
)

REM targetDir
set "targetDir=src\apis\corporate\src"
...
for /r "%targetDir%" %%f in (*service.ts) do (
  ...
  powershell -Command "(Get-Content -Path '%%f') -replace \"apiName = 'Default'\", \"apiName = 'corporate'\" | Set-Content -Path '%%f'"
)
