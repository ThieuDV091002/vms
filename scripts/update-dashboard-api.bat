...
if "%skipProxy%"=="false" (
  rd /s /q src\apis\dashboard
  abp generate-proxy -t ng --api-name dashboard --entry-point src/apis/dashboard
)

REM targetDir
set "targetDir=src\apis\dashboard\src"
...
for /r "%targetDir%" %%f in (*service.ts) do (
  ...
  powershell -Command "(Get-Content -Path '%%f') -replace \"apiName = 'Default'\", \"apiName = 'dashboard'\" | Set-Content -Path '%%f'"
)
